 ...
 * 编译成功后直接运行就行，遥控器先position模式起飞，然后rviz打点，再切offborad模式即可
 ...
 sdu在2026.03.23进行修改优化，初始启动不会飘，然后测试中发现二次启动会导致无人机yaw角变化90度，已修复，修改后无人机yaw角稳定不变了
 ******************************************************************************************/
#include <ros/ros.h>
...
int main(int argc, char **argv)
{
	...
	while(ros::ok() && (!current_state.connected || !have_odom))
	{
		ros::spinOnce();
		rate.sleep();
	}
...
// 		}
	   ...
	   //take off 1m
		if(!receive)
		{
			...
			current_goal.position.x = hold_x;
			current_goal.position.y = hold_y;
			current_goal.position.z = hold_z;
			current_goal.yaw = hold_yaw;

			ROS_INFO_THROTTLE(2.0, "悬停在固定点");
		}
     ...
     //if receive plan in rviz, the EGO plan information can input mavros and vehicle can auto navigation
		if(receive)//触发后进行轨迹跟踪
		{
			...
			double vx = (ego_pos_x - position_x) * 0.5;
			double vy = (ego_pos_y - position_y) * 0.5;
			double vz = (ego_pos_z - position_z) * 0.5;

			double vmax = 0.7;  // 最大速度 m/s
			//V1修改，限制速度为0.8m/s
			vx = std::max(std::min(vx,  vmax), -vmax);
			vy = std::max(std::min(vy,  vmax), -vmax);
			vz = std::max(std::min(vz,  vmax), -vmax);
			...
			ROS_INFO_THROTTLE(0.5, "EGO规划速度：speed=%.2f vx=%.2f vy=%.2f vz=%.2f",
                  std::sqrt(current_goal.velocity.x*current_goal.velocity.x +
                            current_goal.velocity.y*current_goal.velocity.y),
                  current_goal.velocity.x, current_goal.velocity.y, current_goal.velocity.z);
